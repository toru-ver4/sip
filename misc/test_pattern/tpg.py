...
def gen_ramp_bar(width=1920, height=1080,
                 color=const_default_color, direction='h'):
    if direction == 'h':
        x = np.arange(width) / (width - 1)
        r, g, b = [x * c_val for c_val in color]
...
def gen_saturation_bar(width=1920, height=1080,
                       color=const_default_color, direction='h',
                       offset=1.0):
    slope = color - offset
    inv_color = 1 - color
    ...
    if direction == 'h':
        ...
        saturation_line = np.dstack((r, g, b))
        saturation_bar = np.vstack([saturation_line for idx in range(height)])

        cv2.imshow('preview', saturation_bar[:, :, ::-1])
        cv2.waitKey(0)
        ...
        x = np.arange(height) / (height - 1)
        r, g, b = [(x * s_val) + offset for s_val in slope]
        saturation_line = np.dstack((r, g, b)).reshape((height, 1, 3))
        saturation_bar = np.hstack([saturation_line for idx in range(width)])
...
if __name__ == '__main__':
    # gen_ramp_bar(width=1920, height=1080,
    #              color=np.array([1.0, 1.0, 0.0]), direction='v')
    gen_saturation_bar(width=1920, height=1080,
                       color=np.array([1.0, 0.7, 0.3]), direction='v', offset=1.0)
